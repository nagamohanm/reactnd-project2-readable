...
Now you can rename `src/Home.css` to `src/App.scss` and run `npm run watch-css`. The watcher will find every Sass file in `src` subdirectories, and create a corresponding CSS file next to it, in our case overwriting `src/Home.css`. Since `src/Home.js` still imports `src/Home.css`, the styles become a part of your application. You can now edit `src/App.scss`, and `src/Home.css` will be regenerated.
